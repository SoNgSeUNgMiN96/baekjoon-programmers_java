...
public class BOJ2075_ForSubmission{
...
public class BOJ2075 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String heapNameTemp = "MaxHeap";
        heapImplementation.makeHeap(heapNameTemp);
        ...
        for (int i=0;i<N;i++){
            temp = br.readLine().split(" ");
            for (int j=0;j<N;j++) {
                heapImplementation.insertMaxHeap(heapNameTemp, Integer.parseInt(temp[j]));
            }
        }

        for (int i=0;i<N;i++) {
            answer = heapImplementation.deleteRootMaxHeap(heapNameTemp);
        }
        System.out.println(answer);
