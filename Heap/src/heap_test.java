...
public class heap_test {
    ...
    public static int deleteRootAbsHeap(String heapName){
        ArrayList heap = heapMap.get(heapName);
        int index = (int)heap.get(0),answer;
        if(index ==0)
            return Integer.MIN_VALUE;
        ...
        for (int i=1;i*2<index;){
            if (i*2+1<index){
                if(Math.abs((int)heap.get(i*2))>Math.abs((int)heap.get(i*2+1))){
            ...
            if( Math.abs((int)heap.get(i))>Math.abs((int)heap.get(i*2))|| ( Math.abs((int)heap.get(i)) == Math.abs((int)heap.get(i*2+1)) && (int)heap.get(i)>(int)heap.get(i*2) )  ) {
                swap(heap, i, i * 2);
                i *= 2;
            }
            else break;
    ...
    public static int getIndexMaxHeap(String heapName) {
        ArrayList heap = heapMap.get(heapName);
    ...
    public static void main(String[] args) throws Exception {
        String heapNameTemp = "firstMaxHeap";
        ...
        for (int i=0;i<N;i++){
            ...
            if (temp==0){
                if(getIndexMaxHeap(heapNameTemp)!=0)
                    System.out.println(deleteRootMaxHeap(heapNameTemp));
                else
                    System.out.println(0);
            }else
                insertMaxHeap(heapNameTemp,temp);
        }
    }
